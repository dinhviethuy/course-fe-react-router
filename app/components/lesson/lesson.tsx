...
export default function Lesson() {
  ...
  const { register, handleSubmit, formState: { errors }, setValue, watch, control } = useForm({
    ...
    resolver: zodResolver(CreateLessonBodySchema.pick({
      title: true,
      description: true,
      videoUrl: true,
      isDraft: true
    ...
    <form onSubmit={handleSubmit(onSubmit, (e) => {
      ...
      <div className=" space-y-10">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between rounded-md border px-4 py-3 bg-muted">
                <span className="text-sm text-muted-foreground">
                  Lưu bài học dưới dạng bản nháp
                </span>
                <Controller
                  name="isDraft"
                  control={control}
              ...
              {errors.isDraft && (
                <p className="text-sm text-red-500">{errors.isDraft.message}</p>
              )}
            </div>
          </CardContent>
        </Card>
        <Card className="shadow-lg">
          ...
          <CardContent>
            <div className="rounded-xl overflow-hidden border relative">
              <UploadVideo register={register} videoUrl="http://localhost:3000/media/static/videos/849998b2-551e-4a48-a784-27c1d3af1614.mp4" setFile={setFile} setValue={setValue} />
        ...
        <Card className="shadow-lg">
          ...
          <CardContent>
            <ScrollArea className="rounded-md">
              <div>
                <ClientOnly>
                  <MarkdownEditor value={watch("description")}
                    onChange={(val) =>
        ...
        <CardFooter className="flex justify-center">
          <Button type="submit" className="cursor-pointer">Tạo bài học</Button>
        </CardFooter>
      </div>
    </form>
