import type { Route } from '.react-router/types/app/routes/client/manage/profile/+types'
    ...
    <div>
      ...
      <div className='space-y-8'>
        <div className='flex flex-col gap-2'>
          ...
          <Input type='text' placeholder='Nhập họ và tên' className='h-10' {...form.register('fullName')} />
          <p className='text-sm text-muted-foreground'>Tên này sẽ được hiển thị trên trang cá nhân của bạn.</p>
        </div>
        <div className='flex flex-col gap-2'>
          <label className='text-sm font-medium'>Email</label>
          <Input type='email' disabled className='h-10' defaultValue={profile?.email} />
        ...
        <div className='flex flex-col gap-2'>
          <div className='flex gap-2'>
            ...
            <Badge variant='default'>{profile?.status?.toUpperCase()}</Badge>
          </div>
          <p className='text-sm text-muted-foreground'>
        ...
        >
          {updateProfileMutation.isPending && <Loader2Icon className='animate-spin mr-2' />}
          Cập nhật tài khoản
        </Button>
      </div>
    </div>
